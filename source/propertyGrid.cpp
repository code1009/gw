...
#define _stprintf  _snprintf // snprintf->_snprintf
#endif
#endif
//===========================================================================
#ifdef _UNICODE
...
static BOOL IsWindowsXPorLower(VOID) {
	...
	ULONGLONG maskCondition = ::VerSetConditionMask(0, VER_MAJORVERSION, VER_LESS_EQUAL);
...
typedef struct tagLISTBOXITEM {
	...
	LPVOID lpUserData;  ///< Additional user data //DWM 2.1: Suggested by user Jakob
...
/// @param lpUserData Optional pointer to user data.  //DWM 2.1: Suggested by user Jakob
///
...
static LPLISTBOXITEM NewItem(LPTSTR szCatalog, LPTSTR szPropName, LPTSTR szCurValue, LPTSTR szMisc, LPTSTR szPropDesc, INT iItemType, LPVOID lpUserData = NULL)
{
	...
	lpItem->lpUserData = lpUserData; //DWM 2.1: added user managed memory - Jakob
...
static VOID DeleteItem(LPLISTBOXITEM lpItem)
{
	...
	{
		CX_DEBUG_TRACEF(TW_THISCODE, "free(lpItem->lpszCurValue) = %p", lpItem->lpszCurValue);
		CX_DEBUG_TRACEF(TW_THISCODE, "free(lpItem->lpszMisc)     = %p", lpItem->lpszMisc);
	}
...
{
	//DWM 2.1: Added to fix possible crash on application exit 
...
//===========================================================================
	...
	*/	
	...
	//DWM 2.1: added pgi->lpUserData - Jakob
	lpi = NewItem(pgi->lpszCatalog, pgi->lpszPropName,
...
static LRESULT Grid_OnGetItemData(INT iItem)
{
	...
	if (NULL != pItem)
	{
		if (PIT_CATALOG != pItem->iItemType)
		{
			...
			pgi.lpUserData = pItem->lpUserData; //DWM 2.1: added pgi->lpUserData - Jakob
...
static VOID Grid_OnResetContent(VOID)
{
	...
	//DWM 2.1: Added to fix dangling pointer that leads to a crash 
	// under certain circumstances - Jakob
...
///
/// @param iItem The zero-based index of the item to select, or ? to clear the selection.
///
/// @returns LRESULT If an error occurs, the return value is LB_ERR.
///                   If the index parameter is ?, the return value is LB_ERR
...
HWND New_PropertyGrid(HWND hParent, DWORD dwID)
{
	...
	hPropertyGrid = CreateWindowEx(0, g_szClassName, _T(""),
		 WS_CHILD | WS_TABSTOP | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, 0, 0, 0, 0, hParent, (HMENU)dwID, hinst, NULL);
